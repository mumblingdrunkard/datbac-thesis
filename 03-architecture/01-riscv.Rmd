...
While we could likely avoid using CSRs completely, we chose to use them to stay as close as possible to the specification.
...
To prevent live-lock, RISC-V has architectural guarantees that a LR/SC-pair will eventually be successful as long as the SC instruction comes within 16 instructions after the LR instruction\ [@rv5vol1, section 8.2].
...
The exceptions are for jumps caused by branches or by unconditional jumps, or traps.
