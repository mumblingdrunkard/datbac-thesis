...
The tiniest microcontrollers in a modern car are likely running some kind of real time operating system (\acrshort{rtos}).
...
Thanks to a series of clever abstractions we think about the machine as a useful tool for running applications and not a pile of copper and silicon through which current is passed.
...
Those skilled in _The Art of Computer Programming_ @taocp can usually be oblivious to the inner workings of the machine.
...
The programmer doesn't worry about how, when a system call is made, the \acrshort{cpu} executes a trap to escalate privileges and return control from user/application-mode to the underlying system.
